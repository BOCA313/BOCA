...
def get_objective_score(independent):
    ...
    cmd2 = ###compile a program with generated configuration###'gcc -O2 ' + opt + ' -I utilities -I linear-algebra/kernels/2mm utilities/polybench.c linear-algebra/kernels/2mm/2mm.c -lm -DPOLYBENCH_TIME -o 2mm_time'
    cmd5 = ###execute a program###'./2mm_time'
    speedups = []
    step = 0
    while (len(speedups) < 6):
        ...
        de = end - begin
        ###compile a program with -O3###os.system('gcc -O2 -funswitch-loops -ftree-vectorize -fpredictive-commoning -fipa-cp-clone -finline-functions -fgcse-after-reload -I utilities -I linear-algebra/kernels/2mm utilities/polybench.c linear-algebra/kernels/2mm/2mm.c -lm -DPOLYBENCH_TIME -o 2mm_time')
        begin = time.time()
        os.system(cmd5)
        end = time.time()
        nu = end - begin
